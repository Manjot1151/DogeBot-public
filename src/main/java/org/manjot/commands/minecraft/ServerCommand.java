...
public class ServerCommand extends Command implements CommandListener {
    public ServerCommand() {
        this.setName("server")
                .setAliases("minecraftserver", "mcserver")
                .setDescription("Get info about a minecraft server")
                .setUsage("server <ip>")
                .setType(CommandType.MINECRAFT);
    }

    private static String api = "https://api.mcsrvstat.us/2/";
    private static String serverIp = "";

    public static MessageEmbed get(String ip) throws Exception {
        serverIp = ip;
        JsonObject json = Utils.readJsonUrl(api + ip).getAsJsonObject();
        Gson gson = new Gson();
        ServerInfo server = gson.fromJson(json, ServerInfo.class);
        return getAsEmbed(server);
    }

    private static MessageEmbed getAsEmbed(ServerInfo server) {
        EmbedBuilder embed = Utils.getDefaultEmbed();
        if (!server.getOnline()) {
            embed.setTitle(serverIp);
            embed.addField("Status", "```" + server.getStatus() + "```", true);
            embed.addField("Players", "```arm\n0/0```", true);
            embed.addField("IP", "```arm\n" + server.getIp() + "```", false);
            embed.setThumbnail(server.getIcon());
        } else {
            embed.setTitle(server.getHostName());
            embed.addField("Status", "```" + server.getStatus() + "```", true);
            embed.addField("Players", "```arm\n" + server.getPlayerCount() + "```", true);
            embed.addField("IP", "```arm\n" + server.getIp() + "```", false);
            embed.addField("Version", "```arm\n" + server.getVersion() + "```", false);
            embed.addField("Motd", "```arm\n" + server.getMotd() + "```", false);
            embed.setThumbnail(server.getIcon());
        }
        return embed.build();
    }
...
class ServerInfo {
    ...
    public String getMotd() {
        String motdString = "";
        for (int i = 0; i < motd.get("clean").getAsJsonArray().size(); i++) {
            motdString += motd.get("clean").getAsJsonArray().get(i).getAsString();
        }
        return motdString;
    }
    ...
    public String getPlayerCount() {
        return players.get("online").getAsInt() + " / " + players.get("max").getAsInt();
    }
    ...
    public String getIcon() {
        return "https://api.mcsrvstat.us/icon/" + hostname;
    }
 ...
 * "          SKYBLOCK CRYSTAL HOLLOWS!"
 * ],
 * "html": [
 * "             <span style=\"color: #55FF55\">Hypixel Network  <\/span><span style=\"color: #FF5555\">[1.8-1.17]<\/span>"
 * ,
 * "          <span style=\"color: #AA00AA\"><span style=\"font-weight: bold;\">SKYBLOCK CRYSTAL HOLLOWS!<\/span><\/span>"
 * ]
 * },
 * "players": {
 * "online": 94280,
